...
// compatible ctor
SensorManager::SensorManager() : mSensorList(0), mOpPackageName("") {
...
SensorManager& SensorManager::getInstance() {
  return &SensorManager::getInstanceForPackage(
      String16("MtkCam/SensorListener"));
}
}
#endif  // MTK_HARDWARE
...
SensorManager& SensorManager::getInstanceForPackage(
  ...
  if (iterator != sPackageInstances.end()) {
    ...
    // all packages in the UID. The caveat is that the operation may be
    // attributed to the wrong package and stats based on app ops may be
    // slightly off.
    if (opPackageName.size() <= 0) {
      ...
      if (binder != 0) {
        ...
        interface_cast<IPermissionController>(binder)->getPackagesForUid(
            uid, packages);
        if (!packages.isEmpty()) {
...
SensorManager::~SensorManager() { free(mSensorList); }
...
#endif
{
  ...
  if (initSensorManager) {
    // try for 300 seconds (60*5(getService() tries for 5 seconds)) before
    // giving up ...
    ...
    LOG_ALWAYS_FATAL_IF(mSensorServer.get() == NULL,
                        "getService(SensorService) NULL");
    ...
    mSensorList = static_cast<Sensor const**>(malloc(count * sizeof(Sensor*)));
...
Sensor const* SensorManager::getDefaultSensor(int type) {
  ...
  if (assertStateLocked() == NO_ERROR) {
    ...
    // For the following sensor types, return a wake-up sensor. These types are
    // by default defined as wake-up sensors. For the rest of the sensor types
    // defined in sensors.h return a non_wake-up version.
